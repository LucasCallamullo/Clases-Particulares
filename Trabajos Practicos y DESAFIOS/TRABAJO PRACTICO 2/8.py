...
def calcular_importe_final(precio_envio, porc, forma_pago):
    # importe = precio_envio
    precio_envio += (precio_envio * porc) / 100

    if forma_pago == 1:     # significa que era efectivo y un %10 de descuento
        precio_envio -= (precio_envio * 10) / 100

    return int(precio_envio)
...
def principal():
    ...
    for linea in m:
        if primera_linea:
            ...
            # ESTOYT FUERA DE HC Y SC

            #el menor importe pagado por un envio a brasil
            if cod_postal == 0:
