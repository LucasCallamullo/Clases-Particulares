...
# Opcion 4
def opcion4(v_envios, d, e):
    pos = -1
    for i in range(len(v_envios)):
        if v_envios[i].direccion == d and v_envios[i].tipo == e:
            pos = i
            break
    return pos
...
def opcion6(v_envios, tc):
    ...
    for i in range(len(v_cont)):
        print("Tipo de Envio:", i, "tiene una cantidad de envíos de:", v_cont[i])
...
def opcion7(v_envios, tc):
    ...
    for i in range(len(v_acum)):
        print("Tipo de Envio:", i, "tiene el importe acumulado de:", v_acum[i])
...
def opcion8(v_acum_opcion7):
    ...
    print("El Mayor Importe Acumulado es del Tipo de Envio:", tipo, "con un Acumulado de:", may)

    porc = 100 * may / acum_total
    print("El Porcentaje del monto mayor por sobre el total es:", int(porc))


# Opcion 9
def calcular_promedio(v_envios):
    acum = 0
    cont = 0
    for i in v_envios:
        importe = i.calcular_importe()
    ...
    print("El Promedio de los Importes de los Envios es:", int(prom))
    return prom
