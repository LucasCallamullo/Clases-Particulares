...
# PUNTO 14
def calcular_promedio(acum_importe_bsas, cont_env_bsas):
    prom = 0
    if cont_env_bsas > 0:
        prom = int(acum_importe_bsas / cont_env_bsas)
    return prom


# PUNTO 13
def calcular_porcentaje(total_envios, envios_internacionales):
    porc = 0
    if total_envios > 0:
        porc = (envios_internacionales * 100) / total_envios
    return int(porc)
...
def validar_direccion(direccion):
    ...
    for i in direccion:
        if i != " " and i != ".":
            ...
            if not "A" <= i.upper() <= "Z" and not "0" <= i <= "9":             # si no esta la i entre la a y la z
                valido = False
    ...
    for linea in m:
        ...
        if cont_lineas == 1:
            ...
            if control == "Hard Control":
                es_valida_direc = validar_direccion(direccion)      # return True si es valido, return False si NO es valido
                ...
                if es_valida_direc:
                    ...
                    # Punto 4
                    importe = calcular_importe(destino, tipo_envio, cp, forma_pago)         # final
